                    ...
                    {isLabelControlsHorizontal && (
                        <div key={controlIndex} className={`control-${key} ${getContainerColumnClassName(setting, asString(cssDefaults.cssClassInitial))}`}>
                            <div className={asString(cssMap.horizontalControlsContainer)}>
                                {doRenderLabel(setting) && <label htmlFor={setting.name} className={`${asString(cssMap.horizontalLabel)} ${getControlClassName(setting, parentSetting, LABEL_INITIAL, asString(cssDefaults.cssClassInitial))}`}>{setting.label}</label>}
